...
public class BTC_ConnectionManager
{
    ...
    public final void updateTitle(final boolean isConnected)
    {
        ...
        if (isConnected)
        {
            if (loginName == null)
            {
                title = String.format("spaceclient %s // %s:%d", BukkitTelnetClient.VERSION_STRING, hostname, port);
            }
            else
            {
                title = String.format("spaceclient %s // %s@%s:%d", BukkitTelnetClient.VERSION_STRING, loginName, hostname, port);
            }
        }
        else
        {
            title = String.format("spaceclient %s // Idle", BukkitTelnetClient.VERSION_STRING);
        }
